...
Component({
    ...
    methods: {
        ...
        drawOpponents(ctx, view, metrics) {
            view.opponents.forEach((player) => {
                const relative = ((player.seatIndex - view.playerIndex) + 4) % 4;
                if (relative === 2)
                    this.drawTopOpponent(ctx, player, view, metrics);
                if (relative === 3)
                    this.drawSideOpponent(ctx, player, view, metrics, 'left');
                if (relative === 1)
                    this.drawSideOpponent(ctx, player, view, metrics, 'right');
            });
        },
        drawTopOpponent(ctx, player, view, metrics) {
            const tileW = 25;
            const tileH = 36;
            ...
            for (let i = 0; i < count; i += 1)
                this.drawTileBack(ctx, x + i * 17, y, tileW, tileH);
            this.drawTiles(ctx, player.melds?.flatMap((meld) => meld.tiles) || [], x, y + 42, 24, 34);
        },
        drawSideOpponent(ctx, player, view, metrics, side) {
            const x = side === 'left' ? metrics.width * 0.08 : metrics.width * 0.92 - 25;
            const y = metrics.height * 0.22;
            this.drawPlayerPill(ctx, side === 'left' ? x + 34 : x - 126, y - 46, player, view.currentPlayer === player.seatIndex, view.dealer === player.seatIndex);
            const count = Math.min(player.handCount, 13);
            for (let i = 0; i < count; i += 1)
                this.drawTileBack(ctx, x, y + i * 14, 25, 36);
            this.drawTiles(ctx, player.melds?.flatMap((meld) => meld.tiles) || [], side === 'left' ? x + 34 : x - 34, y + 8, 22, 31, true);
        },
        drawDiscards(ctx, view, metrics) {
            const cx = metrics.width / 2;
            const cy = metrics.height * 0.42;
            ...
            view.opponents.forEach((player) => {
                const relative = ((player.seatIndex - view.playerIndex) + 4) % 4;
                if (relative === 2)
                    this.drawTiles(ctx, player.discards.slice(-8), cx - 96, cy - 118, 23, 32);
                if (relative === 3)
                    this.drawTiles(ctx, player.discards.slice(-6), cx - 190, cy - 58, 22, 31, true);
                if (relative === 1)
                    this.drawTiles(ctx, player.discards.slice(-6), cx + 168, cy - 58, 22, 31, true);
            });
        },
        drawSelfHud(ctx, view, metrics) {
        ...
        drawHand(ctx, hand, legalTiles, selectedTile, metrics, handRects) {
            const gap = 4;
        ...
        drawTiles(ctx, tiles, x, y, tileW, tileH, vertical = false) {
        ...
        drawTile(ctx, tile, x, y, w, h) {
            ...
            const img = this.getImage((0, tile_utils_1.getTileImage)(tile));
            if (img)
                ctx.drawImage(img, x + 3, y + 3, w - 6, h - 6);
            else
                this.drawTileFallback(ctx, tile, x, y, w, h);
            if (tile === 18) {
        ...
        drawTileFallback(ctx, tile, x, y, w, h) {
            ctx.fillStyle = '#17231d';
            ctx.font = `700 ${Math.max(10, w * 0.34)}px sans-serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText((0, tile_utils_1.getTileLabel)(tile), x + w / 2, y + h / 2);
        },
        ...
        getImage(src) {
            ...
            if (cached)
                return cached;
            if (state.loadingImages.has(src))
                return null;
